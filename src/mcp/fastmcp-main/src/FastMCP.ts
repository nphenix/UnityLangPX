...
> extends FastMCPSessionEventEmitter {
  ...
  constructor({
    ...
    // 创建完整的服务器信息对象，确保包含所有必需的属性
    const serverInfo = {
      name: name,
      version: version,
      // 添加vendor属性以防止undefined错误
      vendor: "UnityLangPX",
  ...
  private async _safeToJsonSchema(parameters: any): Promise<any> {
    try {
      // Deep clone and clean the parameters object to remove any undefined/null values
      const cleanedParameters = this._cleanParameters(parameters);
      
      // Validate that the cleaned parameters have the required structure
      if (!cleanedParameters ||
          !cleanedParameters["~standard"] ||
          !cleanedParameters["~standard"].shape) {
        throw new Error("Invalid parameters structure");
      }

      return await toJsonSchema(cleanedParameters);
    } catch (error) {
      this.#logger.warn(`Failed to convert parameters to JSON schema:`, error);
  ...
  private _cleanParameters(parameters: any): any {
    if (!parameters) return null;
    
    // Deep clone the parameters object
    const cleaned = JSON.parse(JSON.stringify(parameters));
    
    // Clean the shape properties
    if (cleaned["~standard"] && cleaned["~standard"].shape) {
      const shape = cleaned["~standard"].shape;
      
      for (const [, paramDef] of Object.entries(shape)) {
        if (paramDef && typeof paramDef === "object") {
          const typedParamDef = paramDef as any;
          
          // Remove undefined/null values from constraints
          if (typedParamDef.constraints && typeof typedParamDef.constraints === "object") {
            const constraints = typedParamDef.constraints;
            ...
            if (Object.keys(typedParamDef.constraints).length === 0) {
              delete typedParamDef.constraints;
            }
          }
          
          // Clean other properties
          if (typedParamDef.defaultValue === null || typedParamDef.defaultValue === undefined) {
            delete typedParamDef.defaultValue;
          }
          
          // Ensure typeName is present and valid
          if (!typedParamDef.typeName || typeof typedParamDef.typeName !== "string") {
            typedParamDef.typeName = "string";
          }
          
          // Ensure isOptional is a boolean
          if (typeof typedParamDef.isOptional !== "boolean") {
            typedParamDef.isOptional = false;
          }
        }
      }
    }
  ...
  private setupToolHandlers(tools: Tool<T>[]) {
          ...
          tools.map(async (tool) => {
            ...
            try {
              inputSchema = tool.parameters
                ? await this._safeToJsonSchema(tool.parameters)
                : {
                    additionalProperties: false,
                    properties: {},
                    required: [],
              ...
              console.warn(`Tool ${tool.name} schema generation failed:`, schemaError);
              // Fallback schema when generation fails
              inputSchema = {
                additionalProperties: false,
                properties: {},
                required: [],
            ...
            // Ensure tool response contains all required properties
            const toolResponse = {
            ...
            // Validate tool response object to ensure no undefined properties
            (Object.keys(toolResponse) as Array<keyof typeof toolResponse>).forEach(key => {
              if (toolResponse[key] === undefined || toolResponse[key] === null) {
                console.warn(`Tool ${tool.name} property ${String(key)} is undefined or null, setting to default`);
                (toolResponse as any)[key] = "";
              }
            });
    ...
    this.#server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      if (tool.parameters) {
        let parsed;
        try {
          parsed = await tool.parameters["~standard"].validate(
            request.params.arguments,
          );
        } catch (schemaError) {
          console.warn(`工具 ${tool.name} 的参数schema验证失败:`, schemaError);
